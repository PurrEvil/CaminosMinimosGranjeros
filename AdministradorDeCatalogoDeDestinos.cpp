...
bool AdministradorDeCatalogoDestinos::estaElArribo(Lista<Arribo*>* listaArribos, std::string destinoLlegada, std::string cultivo){

	bool encontro = false;

	listaArribos->iniciarCursor();

	while(listaArribos->avanzarCursor() && !encontro){
		Arribo* arriboActual = listaArribos->obtenerCursor();
		if(arriboActual->obtenerArribo() == destinoLlegada && arriboActual->obtenerCultivo() == cultivo){
			encontro = true;
		}
	}
...
void AdministradorDeCatalogoDestinos::completarCatalogo(Lista<Viaje*>* destinos){
	...
	Lista<Arribo*>* arribosOrigen;
	destinos->iniciarCursor();
	bool encontro = false;
	while(destinos->avanzarCursor() && !encontro){
		if(destinos->obtenerCursor()->obtenerSalidaDelViaje() == "ALMACEN"){
			arribosOrigen = destinos->obtenerCursor()->obtenerArribos();
			encontro = true;
		}
	}
	destinos->iniciarCursor();
	while(destinos->avanzarCursor()){
		Viaje* viajeActual = destinos->obtenerCursor();
		Lista<Arribo*>* arribosActual = viajeActual->obtenerArribos();
		arribosActual->iniciarCursor();
		while(arribosActual->avanzarCursor()){
			std::string destinoLlegada = arribosActual->obtenerCursor()->obtenerArribo();
			std::string cultivo = arribosActual->obtenerCursor()->obtenerCultivo();
			unsigned int costo = (*arribosActual->obtenerCursor()->obtenerCosto());

			if(!this->estaElArribo(arribosOrigen, destinoLlegada, cultivo)){


				arribosOrigen->agregar(new Arribo(destinoLlegada,INF, cultivo));
			}
...
/*
	...
	for(unsigned int posicion = 2; posicion <= destinos->contarElementos(); posicion++){
		...
		while(arribosOrigen->avanzarCursor()){
